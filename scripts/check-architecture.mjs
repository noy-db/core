...
const KERNEL_SURFACE_BUDGET = {
  ...
  // plus its rationale.
  // Bumped 4371→4373 (2026-09-05) over two changes, each ONE line and each an
  // import; the bodies they serve are net-neutral and the capabilities live in
  // their own modules.
  //   #1417 keyset paging — the cache is a `GenerationMap`
  //     (kernel/generation-map.ts), a Map that stamps its own mutations so a
  //     memo over it can tell when it moved. The alternative was bumping a
  //     counter at the ELEVEN sites that write this cache, which trades one
  //     import line for a permanent invitation to a stale-read bug: miss one
  //     site and `page()` serves rows from before a write.
  //   #1419 warm-list re-decoration — the Via `present()` pass is memoized per
  //     (record identity, locale variant) in `kernel/present-cache.ts`, which
  //     carries the capability and its stated soundness assumption.
  // ⚠️ These two landed as separate PRs that each raised the ceiling to 4372 and
  ...
  // Bumped 2179→2182 (2026-08-15, #1042 merge-authority wiring): three lines —
  // one import, one construction, two call-site passes. Pure wiring, and it
  // cannot move. The engine cannot build this itself (`with-sync` is DEK-free
  // and check:architecture keeps it so), and it cannot register on the
  ...
  // Bumped 2420→2440 (2026-09-23, core#101 follow-up): ZERO new statements — entirely the doc
  ...
  'packages/hub/src/kernel/noydb.ts': 2440,  // see the #1121 note above
 ...
 * in `noy-db/on`. Nine left on 09-01. `on-shamir` stayed two more
...
//
// History: introduced 2026-09-02 as a RATCHET over ten grandfathered files
// (38 calls); driven to zero one file per commit and turned into a ban on
// 2026-09-11 (capsule seam spec, Stage A). Do not reintroduce a map — every
// primitive a service needs has a landing place on the barrel
// (`encryptBytes`/`decryptBytes`, `exportDekSet`/`importDekSet`,
// `importTransferKey`, `generateEphemeralKey`, `mintCanary`/`checkCanary`,
// `hkdfAesGcmKey`, `derivePresenceTagKey`/`hmacSignHex`, the recipient
// group); add the primitive there instead.
//
...
const PRE_EXISTING_BODY_ACCESS = new Map([
  ...
  ['packages/hub/src/with-shape/blobs/blob-compaction.ts', 0],
  ...
  ['packages/hub/src/with-shape/links/link-set.ts', 1],
  ['packages/hub/src/with-shape/persisted-schemas/storage.ts', 0],
...
//
// The ten `as-*` packages moved to `noy-db/as`. The check was deleted
