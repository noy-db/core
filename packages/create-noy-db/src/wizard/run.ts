...
import { NOYDB_IN_VERSION, ownVersion, renderTemplate, templateDir, type RenderTokens } from './render.js'
import type {
  WizardAdapter,
  WizardOptions,
  WizardResult,
...
/**
 * The original fresh-project path, factored out of the
 * main entry so the augment branch above can coexist. Behavior
 * is unchanged from except the return shape now includes
...
async function runFreshMode(
  ...
  const template: WizardTemplate = yes
    ? options.template ?? 'nuxt-default'
    : await promptTemplate(options.template, msg)

  const syncAdapter: WizardAdapter | 'none' = yes
    ? options.syncAdapter ?? 'none'
    : await promptSyncAdapter(options.syncAdapter, adapter, msg)
...
/**
 * The new augment-existing-project path. Runs magicast
...
async function promptSyncAdapter(
  initial: WizardAdapter | 'none' | undefined,
  primary: WizardAdapter,
  _msg: WizardMessages,
): Promise<WizardAdapter | 'none'> {
  if (initial !== undefined) return initial
  const choices = (['none', 'memory', 'file', 'browser'] as const).filter(
    (x) => x !== primary,
  )
  const labels: Record<WizardAdapter | 'none', string> = {
    none: 'No sync — local only',
    memory: 'In-memory (testing / CI)',
    file: 'JSON files (USB / shared drive)',
    browser: 'IndexedDB (other tabs / PWAs)',
  }
  const result = await p.select<WizardAdapter | 'none'>({
    message: 'Add a sync target?',
    options: choices.map((value) => ({ value, label: labels[value] })),
    initialValue: 'none',
  })
  if (p.isCancel(result)) {
    process.exit(1)
...
async function promptTemplate(
  initial: WizardTemplate | undefined,
  _msg: WizardMessages,
): Promise<WizardTemplate> {
  if (initial) return initial
  const result = await p.select<WizardTemplate>({
    message: 'Pick a starter template',
    options: [
      {
        value: 'nuxt-default',
        label: 'Nuxt 4 + Pinia',
        hint: 'SSR + file-based routing (recommended default)',
      },
      {
        value: 'vite-vue',
        label: 'Vite + Vue 3 + Pinia',
        hint: 'Client-side SPA, shorter install graph',
      },
      {
        value: 'vanilla',
        label: 'Vite + TypeScript',
        hint: 'No framework, smallest footprint',
      },
      {
        value: 'electron',
        label: 'Electron + Vue 3',
        hint: 'Desktop app with to-file adapter — USB workflow',
      },
    ],
    initialValue: 'nuxt-default',
  })
  if (p.isCancel(result)) {
    process.exit(1)
  }
