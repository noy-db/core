...
import type { NoydbStore, EncryptedEnvelope, VaultSnapshot, Conflict } from '../src/kernel/types.js'
...
describe('conflictPolicy (v0.9)', () => {
  ...
  describe('custom merge fn', () => {
    ...
    it('does not affect collections without conflictPolicy', async () => {
      ...
      const db = await createNoydb({ store: local, sync: remote, user: 'u', syncStrategy: withSync(), encrypt: false, conflict: 'remote-wins' })
      const comp = await db.openVault(COMP)
