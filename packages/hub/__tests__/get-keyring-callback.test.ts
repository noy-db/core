...
describe('NoydbOptions.getKeyring (issue #5)', () => {
  ...
  it('issue #6: openVault succeeds on a cleared data store when getKeyring callback returns a valid keyring', async () => {
    ...
    const v1 = await db1.openVault('pilot1')
    await v1.collection<Note>('notes').put('n1', { title: 'hello' })

    // Capture the keyring as it would be held by an "auth store" (e.g. pilot1-auth_noydb).
    const authKeyring = await loadKeyring(adapter, 'pilot1', { userId: 'alice', secret: 'p' })

    // Simulate the user deleting pilot1_noydb in DevTools — use a fresh empty adapter.
    const clearedAdapter = inlineMemory()

    // The app's getKeyring callback reads from the still-intact auth store
    // (pilot1-auth_noydb) and returns the keyring without touching the data store.
    ...
    const v2 = await db2.openVault('pilot1')
