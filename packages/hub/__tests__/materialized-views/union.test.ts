...
describe('UNION MV — combined with multi-key groupBy (#165 + #166)', () => {
  it('pilot1 canonical monthly-VAT shape: union(taxReceipts, creditNotes) + groupBy(clientId, period)', async () => {
    interface Pilot1TaxReceipt extends Record<string, unknown> {
      id: string
      clientId: string
      issuedAt: string
      paidServicesVat: number
    }
    interface Pilot1CreditNote extends Record<string, unknown> {
      id: string
      clientId: string
      issuedAt: string
      paidServicesVat: number
    }
    interface Pilot1MonthlyVatRow extends Record<string, unknown> {
      clientId: string
      period: string
      vat: number
    }

    const monthlyOutputVat = withMaterializedView<Pilot1MonthlyVatRow>({
      name: 'monthlyOutputVat',
      unionSources: [
        {
          collection: 'taxReceipts',
          map: r => {
            const tr = r as unknown as Pilot1TaxReceipt
            return {
        ...
        },
        {
          collection: 'creditNotes',
          map: r => {
            const cn = r as unknown as Pilot1CreditNote
            return {
    ...
    const db = await createNoydb({
      store: toMemory(),
      user: 'alice',
      secret: 'mv-union-multikey-pilot1-secret-2026',
      materializedViewStrategies: [monthlyOutputVat],
      reduceStrategy: withReduce(),
    })
    const vault = await db.openVault('demo')

    const receipts = vault.collection<Pilot1TaxReceipt>('taxReceipts')
    const notes = vault.collection<Pilot1CreditNote>('creditNotes')
    ...
    const out = vault.collection<Pilot1MonthlyVatRow & { _materializedFrom?: unknown }>('monthlyOutputVat')
...
describe('UNION MV — queryHash sensitivity (#165 pilot1 review)', () => {
