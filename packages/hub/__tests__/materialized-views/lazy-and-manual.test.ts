...
describe('MV lazy lifecycle + vault.refreshView (#151)', () => {
  ...
  it('refreshView throws on unknown MV name', async () => {
    ...
    expect(await vault.refreshView('nonexistent')).toEqual({ written: 0, skipped: 0, deleted: 0, failed: 0, residueUndecodable: [], residueDeclined: [] })
  ...
  it('list() triggers lazy-MV resolve-on-read (pilot1-review of #157)', async () => {
    const lazyMV = withMaterializedView<Item>({
      name: 'red-items',
      query: (db) => db.collection<Item>('items').query().where('tag', '==', 'red'),
      rowKey: (r) => r.id,
  ...
  it('refreshView returns executor counts with deleted in the shape (pilot1-review of #157)', async () => {
