...
**Now a re-export of `@noy-db/ports/to`, and deprecated in favour of it.** The six conformance kits were consolidated into one package, `@noy-db/ports`, one subpath per hub port. **This is not a break:** every function this package exported is still exported from here, unchanged in name and signature, and the peers are the same (`@noy-db/hub`, `vitest`). New code should import `@noy-db/ports/to`; this name stays published so an existing pin keeps working, and it will be deprecated on npm once the family's satellites have moved.
