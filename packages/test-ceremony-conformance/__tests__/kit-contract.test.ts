...
function runSuite(name: string, ...extra: readonly string[]): { code: number; out: string } {
  try {
    const out = execFileSync('npx', ['vitest', 'run', '--config', CONFIG, name, ...extra], {
      cwd: REPO_ROOT,
      encoding: 'utf8',
      stdio: 'pipe',
    })
...
describe('the ceremony conformance kit, run against ceremonies broken in one named way each', () => {
  it('⛔ FAILS a ceremony with no method guard — the slot-type swap', () => {
    expectRefused('broken-accepts-any-method', [
      /× .*4\. REFUSES a slot belonging to another method/,
      /× .*5\. refuses without mutating the context it was handed/,
    ])
  }, 120_000)

  it('⛔ FAILS a ceremony that wraps a stale DEK set', () => {
    expectRefused('broken-stale-deks', [/× .*6\. wraps the NEW dek set, not the old one/])
  }, 120_000)

  it('SAYS SO IN THE OUTPUT when a fixture omits `unwrap`, rather than passing quietly', () => {
    ...
    const { code, out } = runSuite('no-unwrap', '--reporter=verbose')
